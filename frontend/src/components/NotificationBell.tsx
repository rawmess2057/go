...
import { Bell, FileArrowDown, CheckCircle, Plus, Info } from "@phosphor-icons/react";
import { useNotifications } from "@/hooks/useNotifications";

const typeIcons: Record<string, React.ReactNode> = {
  submission: <FileArrowDown size={16} />,
...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-2xl border border-border bg-surface shadow-xl overflow-hidden z-50">
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-border">
            ...
            {items.map((n) => {
              const content = (
                ...
                >
                  <span className="shrink-0 mt-0.5 text-muted-foreground">{typeIcons[n.type] ?? <Info size={16} />}</span>
